        ...
        if params.global_place_flag:
            ...
            for global_place_params in params.global_place_stages:
                ...
                def one_descent_step(Lgamma_step, Llambda_density_weight_step, Lsub_step, iteration, metrics, stop_mask=None):
                    ...
                    if torch.eq(model.density_weight.mean(), 0.0):
                        ...
                        if model.density_weight.size(0) == 1:
                            ...
                            logging.info("density_weight = [%s]" 
                                         % ", ".join(["%.3E" % i for i in model.density_weight.cpu().numpy().tolist()]))
                    
                    if params.frequency_assign:
                        if torch.eq(model.frequency_repulsion_weight.mean(), 0.0):
                            model.initialize_frequency_repulsion_weight(params)
                            if model.frequency_repulsion_weight.size(0) == 1:
                                logging.info("frequency_repulsion_weight = %.6E" % (model.frequency_repulsion_weight.data))
                            else:
                                logging.info("frequency_repulsion_weight = [%s]"
                                    % ", ".join(["%.3E" % i for i in model.frequency_repulsion_weight.cpu().numpy().tolist()]))
                    ...
                    # if params.plot_flag and (iteration % 5 == 0 or iteration == 999):
                ...
                def check_divergence(x, window=50, threshold=0.05):
                    ...
                    wl_mean = np.mean(x[:, 0])
                    wl_ratio = (wl_mean - best_metric[0].hpwl.item()) / best_metric[0].hpwl.item() 
                    overflow_ratio = (overflow_mean - max(params.stop_overflow, best_metric[0].overflow.item())) / best_metric[0].overflow.item()
                ...
                for Lgamma_step in range(model.Lgamma_iteration):
                    ...
                    for Llambda_density_weight_step in range(model.Llambda_density_weight_iteration):
                        ...
                        for Lsub_step in range(model.Lsub_iteration):
                            ...
                            if (len(placedb.regions) == 0
                                ...
                                if overflow_list[-1] > 0.9:  # stuck at high overflow
                                    ...
                                    logging.info(f"Stuck at early stage. Turn on quadratic penalty with double density factor to accelerate convergence")
                                    if overflow_list[-1] > 0.95:  # stuck at very high overflow
                                        ...
                                        logging.info(f"Stuck at very early stage. Turn on entropy injection with noise intensity = {noise_intensity} to help convergence")
                        ...
                        if Llambda_flat_iteration > 1:
                            ...
                            if params.frequency_assign:
                                model.op_collections.update_frequency_repulsion_weight_op(
                                    Llambda_metrics[-1][-1],
                                    Llambda_metrics[-2][-1] if len(Llambda_metrics) > 1 else Lgamma_metrics[-2][-1][-1],
                                    Llambda_flat_iteration,
                                    )
