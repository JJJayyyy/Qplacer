        ...
        ) 
{
    ...
    for (int i = 0; i < num_bins_x*num_bins_y; i += 1) {
        ...
        //     for (int node_idx=0; node_idx<node_in_group[edge_idx].size(); node_idx++) {
        //         ci += 1;
        // ///////////////
        ...
        std::cout << "edge_lengths: ";
        for (const auto& len : edge_lengths) std::cout << len << " ";
        std::cout << std::endl;
        ...
        for (int ci = bin_cells.at(i).size()-1; ci >= 0; --ci) {
            ...
            if (total_bins-1 - ci == edge_lengths[edge_idx]) {     // ci is descending 
                ...
                // std::cout << "check edge_touch_blanks DONE " << std::endl; 
            ...
            if (best_blank_bin_id_y >= 0) {
                ...
                checkBlanksCPU(edge_touch_blanks, "check edge_touch_blanks", true);
                bool first_y = true;
                for (int row_idx = 0; row_idx < edge_touch_blanks.size(); row_idx++) {      // check whether there are touched blanks 
