...
int greedyLegalizationCPU(
	...
	// dreamplacePrint(kINFO, "---------- Greedy  ----------\n");

	for (size_t i = 0; i < num_movable_nodes; ++i) {
		std::cout << "i: " << i << "	init_x: " <<init_x[i]<< ", init_y: " <<init_y[i]<< " | node_size_x: " 
			<<node_size_x[i]<< ", node_size_y: " <<node_size_y[i]
		 	<< " | x: " <<x[i]<< ", y: " <<y[i]<< std::endl;
	}
	std::cout << "xl: " <<xl<< "; yl: " <<yl<< "; xh: " <<xh<< "; yh: " <<yh<< std::endl;
	std::cout << "site_width: "<< site_width<< " | row_height: " <<row_height 
			<< " | num_bins_x: "<< num_bins_x<< "; num_bins_y: " <<num_bins_y 
			<< " | num_nodes: "<< num_nodes<< "; num_movable_nodes: "<< num_movable_nodes<< std::endl;
	
	std::cout << "node_in_group: " << std::endl;
	for (const auto& row : node_in_group) {
		for (const auto& idx : row) std::cout << idx << " ";
		std::cout << std::endl;
	}
