...
///
/// @param init_x database for legalization.
template <typename T>
int greedyLegalizationLauncher(LegalizationDB<T> db, 
							const std::vector<std::vector<int>> node_in_group) {
	dreamplacePrint(kINFO, "---------- Greedy legalization ----------\n");

	greedyLegalizationCPU(
		db, db.init_x, db.init_y, db.node_size_x, db.node_size_y, db.x, db.y, 
		db.xl, db.yl, db.xh, db.yh, db.site_width, db.row_height, 
		db.num_bins_x, db.num_bins_y, db.num_nodes, db.num_movable_nodes,
		node_in_group
		);
  	return 0;
}
...
/// @param num_filler_nodes number of filler nodes, filler nodes are in the range of
/// [num_nodes-num_filler_nodes, num_nodes)
at::Tensor greedy_legalization_forward(
    at::Tensor init_pos, at::Tensor pos, at::Tensor node_size_x, at::Tensor node_size_y, 
    at::Tensor node_weights, at::Tensor flat_region_boxes, at::Tensor flat_region_boxes_start,
    at::Tensor node2fence_region_map, double xl, double yl, double xh, double yh, 
	...
	DREAMPLACE_DISPATCH_FLOATING_TYPES(
		pos, "greedyLegalizationLauncher", [&] {
			...
			greedyLegalizationLauncher<scalar_t>(db, node_in_group);
			// db.check_legality();
		});
